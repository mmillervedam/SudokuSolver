...
class Sudoku():
    "Class to keep track of sudoku board."

    # Functions to initialize, maintain and check board state
    ...
    def set_board(self, setup_dict):
        """
        Takes a setup dictionary of coordinates and values
        and loads them on to the given board.
    ...
    def is_valid(self):
        """
        Identify if any row, column or box has duplicates and if any
        empty tile has no possible moves.
        Returns boolean (false if either condition is triggered).
        """
